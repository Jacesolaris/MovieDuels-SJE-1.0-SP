...
extern saberMoveName_t PM_SaberFlipOverAttackMove();
extern qboolean PM_CheckFlipOverAttackMove(qboolean check_enemy);
extern saberMoveName_t PM_SaberJumpForwardAttackMove();
...
static qboolean PM_CheckJump()
{
	...
	if ((pm->ps->client_num < MAX_CLIENTS || PM_ControlledByPlayer())
		...
		&& (pm->gent->client->NPC_class == CLASS_BOBAFETT ||
			...
			pm->gent->client->NPC_class == CLASS_ROCKETTROOPER || rocket_trooper_player(pm->gent)) && !pm->ps->
		hookhasbeenfired)
	{
							...
							&& anim != BOTH_FORCEWALLRUNFLIP_START)
						{
							if (pm->gent && trace.entity_num < ENTITYNUM_WORLD)
							{
									...
									&& !(trace_ent->flags & FL_NO_KNOCKBACK))
								{
									...
									//G_Sound( trace_ent, G_SoundIndex( va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) );
...
void G_StartDash(gentity_t* ent, const int anim)
{
...
qboolean PM_NotWalkingAnim(const int anim)
{
	switch (anim)
	{
	case BOTH_RUN1:
	...
	else if (pm->ps->pm_flags & PMF_BACKWARDS_RUN)
	{
		...
		if (!(pm->cmd.buttons & BUTTON_WALKING))
		{
			...
			else
			{
				...
				{
					if (pm->gent->client->NPC_class == CLASS_REBORN && pm->ps->weapon == WP_BLASTER_PISTOL ||
						pm->gent && pm->gent->weaponModel[1] > 0) //they do this themselves
					{
...
static void PM_BeginWeaponChange(const int weapon)
{
	...
	if (pm->ps->weapon == WP_SABER && pm->ps->ManualBlockingFlags & 1 << MBF_BLOCKING || pm->ps->ManualBlockingFlags
		& 1 << MBF_PROJBLOCKING)
	{
		return;
	}
...
int PM_ReadyPoseForSaberAnimLevelDucked()
{
	int anim;
...
int PM_ReadyPoseForSaberAnimLevelNPC()
{
	int anim;
...
void PM_BlockFatigue(playerState_t* ps, const int new_move)
{
	if (g_SerenityJediEngineMode->integer == 2)
	{
		if (ps->saber_move != new_move)
...
int PM_SaberLockWinAnim(const saberLockResult_t result, const int break_type)
{
	int win_anim = -1;
...
{
	const saberMoveName_t kick_move = g_pick_auto_multi_kick(pm->gent, allow_singles, qfalse);
	if (kick_move != LS_NONE)
	{
...
void pm_melee_move_for_conditions()
{
	if (pm->gent && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent))) //player only restrictions
	{
		if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive() && !pm->ps->saberInFlight)
				...
				else if (PM_SaberInBounce(pm->ps->saber_move) || !PM_SaberInAttack(pm->ps->saber_move))
				{
					...
					if (pm->cmd.buttons & BUTTON_ATTACK)
					{
						...
						else
						{
							...
							while (new_quad == saberMoveData[pm->ps->saber_move].startQuad)
							{
								//player is still in same attack quad, don't repeat that attack because it looks bad,
								new_quad = Q_irand(Q_BR, Q_BL);
							} //else player is switching up anyway, take the new attack dir
							next_move = transitionMove[saberMoveData[pm->ps->saber_move].startQuad][new_quad];
						}
					}
					else
					{
...
void PM_WeaponLightsaber()
{
	...
	if (!delayed_fire)
	{
		...
		else if (!(pm->cmd.buttons & BUTTON_ATTACK) || holding_block || active_blocking || walking_blocking)
		{
			//not attacking
			pm->ps->weaponTime = 0;
			...
			if (curmove >= LS_S_TL2BR && curmove <= LS_S_T2B)
			{
				...
				if (g_SerenityJediEngineMode->integer == 2)
				{
					...
					{
						//perform attack fake
						newmove = pm_returnfor_quad(saberMoveData[curmove].endQuad);
					}
				}
				else
				{
