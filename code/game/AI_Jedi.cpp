...
qboolean npc_is_staff_style(const gentity_t* self)
{
...
qboolean npc_is_dual_style(const gentity_t* self)
{
...
qboolean jedi_cultist_dark_force_user(const gentity_t* self)
{
	if (!self || !self->client)
	{
		return qfalse;
		...
		for (int time = cur_time - 25; time <= cur_time + 25 && !hit; time += 25)
		{
			...
			if (trace.fraction < 1.0f)
			{
						...
						NPC_class))
				{
					...
					G_Damage(trace_ent, NPC, NPC, vec3_origin, trace.endpos, dmg, DAMAGE_NO_KNOCKBACK,
						MOD_SABER); //MOD_MELEE );
					if (trace_ent->client)
					{
						if (!Q_irand(0, 2))
...
void tavion_start_scepter_beam()
{
...
void tavion_start_scepter_slam()
{
...
void tavion_sith_sword_recharge()
{
...
qboolean self_is_gunner(const gentity_t* self)
{
					...
					if (trace.fraction * check_dist <= 32)
					{
						...
						if (trace.fraction >= 1.0f)
						{
							...
							if (allow_wall_flips)
							{
								...
								//so we don't take damage if we land at same height
								self->client->ps.pm_flags |= PMF_JUMPING | PMF_SLOW_MO_FALL;
								if (self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class ==
									CLASS_MANDALORIAN || self->client->NPC_class == CLASS_JANGO || self->client->
...
qboolean jedi_in_no_ai_anim(const gentity_t* self)
{
	if (!self || !self->client)
	{
		//wtf???
...
void jedi_check_jump_evasion_safety(usercmd_t* cmd, const evasionType_t evasion_type)
{
...
qboolean jedi_check_ambush_player()
{
...
qboolean jedi_can_pull_back_saber(const gentity_t* self)
{
...
qboolean jedi_check_kata_attack()
{
						...
						if (TIMER_Done(NPC, "attackDelay"))
						{
								...
								|| !Q_irand(0, 80 - g_spskill->integer * 20))
							{
								...
								case 3:
									...
									{
										if (NPC->client->NPC_class == CLASS_GALEN
											&& (NPC->s.weapon == WP_MELEE || NPC->s.weapon == WP_NONE || NPC->s.
												weapon == WP_SABER && !NPC->client->ps.SaberActive())
											&& NPC->client->ps.groundEntityNum == ENTITYNUM_NONE)
										{
