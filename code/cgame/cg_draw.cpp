...
void CG_DrawDF_GunFatigue(const centity_t* cent)
{
	...
	float fatigue_percent = static_cast<float>(cent->gent->client->ps.BlasterAttackChainCount) / (BLASTERMISHAPLEVEL_MAX
		+ 1);
...
void CG_DrawHealthBar(const centity_t* cent, const float ch_x, const float ch_y, const float ch_w, const float ch_h)
{
	...
	CG_FillRect(x - 1.0f + percent * ch_w, y + 1.0f, ch_w - percent * ch_w, ch_h - 2.0f, c_color);
...
void CG_DrawBlockPointBar(const centity_t* cent, const float ch_x, const float ch_y, const float ch_w, const float ch_h)
{
	...
	//draw the other part greyed out
	CG_FillRect(x - 1.0f + block_percent * ch_w, y + 1.0f, ch_w - block_percent * ch_w, ch_h - 2.0f, c_color);
	...
	const float ch_h)
{
	...
	//draw the other part greyed out
	CG_FillRect(x + fatigue_percent * ch_w, y + 1.0f, ch_w - fatigue_percent * ch_w, ch_h - 2.0f, a_color);
...
void CG_DrawForcePointBar(const centity_t* cent, const float ch_x, const float ch_y, const float ch_w, const float ch_h)
{
	...
	//draw the other part greyed out
	CG_FillRect(x - 1.0f + force_percent * ch_w, y + 1.0f, ch_w - force_percent * ch_w, ch_h - 2.0f, c_color);

	//draw the border (black)
	CG_DrawRect(x, y, ch_w, ch_h, 1.0f, colorTable[CT_BLACK]);

	//now draw the part to show how much health there is in the color specified
	CG_FillRect(x + 1.0f, y + 1.0f, force_percent * ch_w - 2.0f, ch_h - 2.0f, a_color);
}
...
static float CG_DrawFPS(const float y)
{
	...
	const int w = cgi_R_Font_StrLenPixels(s, cgs.media.qhFontMedium, 1.0f);
	cgi_R_Font_DrawString(635 - x_offset - w, y + 2, s, colorTable[CT_LTGOLD1], cgs.media.qhFontMedium, -1, 1.0f);
