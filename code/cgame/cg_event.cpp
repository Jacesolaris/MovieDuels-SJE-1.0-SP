...
void CG_EntityEvent(centity_t* cent, vec3_t position)
{
	...
	case EV_FALL_MEDIUM:
		...
		if (g_entities[es->number].health <= 0)
		{
			...
			|| g_entities[es->number].client && g_entities[es->number].client->ps.forcePowersKnown & 1 <<
			FP_LEVITATION)
		{
	...
	case EV_JEDISPAWN:
		DEBUGNAME("EV_JEDISPAWN");
		{
			...
			gi.trace(&tr, position, playerMins, playerMaxs, dpos, es->number, MASK_SOLID, static_cast<EG2_Collision>(0), 0);
			VectorCopy(tr.endpos, pos);

			cgi_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.teleInSound);

			effect = cgs.media.jedispawn;

			if (tr.fraction == 1)
			{
				break;
			}
			theFxScheduler.PlayEffect(effect, pos, ang);*/
		}
		break;
	...
	case EV_DISINTEGRATION:
	{
		...
		if (cent->gent->owner)
		{
			...
			{
				cent->gent->owner->s.eFlags = disint_effect; //FIXME: |= ?
	...
	case EV_PAIN:
	{
		...
		CG_TryPlayCustomSound(nullptr, es->number, CHAN_VOICE, snd, CS_BASIC);
	...
	case EV_USE_INV_GRAPPLEHOOK:
		DEBUGNAME("EV_USE_INV_GRAPPLEHOOK");
		UseItem(INV_GRAPPLEHOOK);
		break;
