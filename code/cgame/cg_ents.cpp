	...
	const char* tag_name, orientation_t* tag_orient)
{
	...
	MatrixMultiply(entity->axis, lerped.axis, temp_axis);
	MatrixMultiply(temp_axis, const_cast<refEntity_t*>(parent)->axis, entity->axis);
}
...
static void CG_EntityEffects(const centity_t* cent)
{
	...
	if (cent->currentState.loopSound)
	{
		...
		{
			cgi_S_AddLoopingSound(cent->currentState.number, v3_origin, vec3_origin, sfx, chan);
		}
	}
...
{
	static constexpr size_t num_bracket_ents = std::size(cg.bracketedEntities);
...
static void CG_General(centity_t* cent)
{
	...
	*/
	...
	if (cent->currentState.weapon == WP_STUN_BATON && cg_SerenityJediEngineMode.integer == 2)
	{
		int i;
		orientation_t lerped;
	...
	if (cent->currentState.weapon == WP_MELEE && cg_SerenityJediEngineMode.integer == 2)
	{
		int i;
		orientation_t lerped;
...
static void CG_Missile(centity_t* cent)
{
	...
	*/
	...
	if (s1->weapon == WP_STUN_BATON && cg_SerenityJediEngineMode.integer == 2)
	{
		orientation_t lerped;
	...
	if (s1->weapon == WP_MELEE && cg_SerenityJediEngineMode.integer == 2)
	{
		orientation_t lerped;
...
void CG_MatrixEffect(const centity_t* cent)
{
	...
	const auto total_elapsed_time = static_cast<float>(cg.time - cent->currentState.time);
...
void CG_StasisEffectLong(const centity_t* cent)
{
