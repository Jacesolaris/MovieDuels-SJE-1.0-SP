...
int G2_Find_Bone(const CGhoul2Info* ghlInfo, const boneInfo_v& blist, const char* bone_name)
{
	...
	for (size_t i = 0; i < blist.size(); i++)
	{
		...
		const mdxaSkel_t* skel = reinterpret_cast<mdxaSkel_t*>((byte*)ghlInfo->aHeader + sizeof(mdxaHeader_t) + offsets->offsets[blist[
			i].
			boneNumber]);
	...
	const vec3_t offset = nullptr)
{
	...
	if (flags & (BONE_ANGLES_PREMULT | BONE_ANGLES_POSTMULT))
	{
		...
		{
		case NEGATIVE_X:
			newer_angles[1] = angles[2] + 180;
			break;
		case POSITIVE_X:
			newer_angles[1] = angles[2];
			break;
		case NEGATIVE_Y:
			newer_angles[1] = angles[0];
			break;
		case POSITIVE_Y:
			newer_angles[1] = angles[0];
			break;
		case NEGATIVE_Z:
			newer_angles[1] = angles[1] + 180;
			break;
		case POSITIVE_Z:
			newer_angles[1] = angles[1];
			break;
		default:
			break;
		}
		...
		{
		case NEGATIVE_X:
			newer_angles[0] = angles[2];
			break;
		case POSITIVE_X:
			newer_angles[0] = angles[2] + 180;
			break;
		case NEGATIVE_Y:
			newer_angles[0] = angles[0];
			break;
		case POSITIVE_Y:
			newer_angles[0] = angles[0] + 180;
			break;
		case NEGATIVE_Z:
			newer_angles[0] = angles[1];
			break;
		case POSITIVE_Z:
			newer_angles[0] = angles[1];
			break;
		...
		{
		case NEGATIVE_X:
			newer_angles[2] = angles[2];
			break;
		case POSITIVE_X:
			newer_angles[2] = angles[2];
			break;
		case NEGATIVE_Y:
			newer_angles[2] = angles[0];
			break;
		case POSITIVE_Y:
			newer_angles[2] = angles[0] + 180;
			break;
		case NEGATIVE_Z:
			newer_angles[2] = angles[1];
			break;
		case POSITIVE_Z:
			newer_angles[2] = angles[1] + 180;
			break;
		default:
			break;
		}

		Create_Matrix(newer_angles, bone_override);
...
{
	const int index = G2_Find_Bone(ghlInfo, blist, bone_name);
	if (index == -1)
	{
		return qfalse;
	}
...
#if DEBUG_PCJ
	OutputDebugString(va("%8x  %2d %6d   (%6.2f,%6.2f,%6.2f) %d %d %d %d\n", (int)ghlInfo, index, current_time, angles[0], angles[1], angles[2], yaw, pitch, roll, flags));
#endif
...
qboolean G2_Set_Bone_Angles(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* bone_name, const float* angles,
	...
	const int blend_time, const int current_time, const vec3_t offset)
{
	int index = G2_Find_Bone(ghlInfo, blist, bone_name);
	if (index == -1)
	{
		index = G2_Add_Bone(ghlInfo->animModel, blist, bone_name);
	}
	if (index != -1)
	{
		blist[index].flags &= ~(BONE_ANGLES_TOTAL);
...
qboolean G2_Set_Bone_Anim_Index(boneInfo_v& blist, const int index, const int startFrame,
	const int endFrame, const int flags, const float anim_speed, const int current_time,
	const float set_frame, const int ablend_time, const int numFrames)
{
	int mod_flags = flags;
	...
	assert(
		set_frame == -1 || (set_frame >= startFrame && set_frame < endFrame) || (set_frame > endFrame && set_frame
			<= startFrame + 1));
	...
	{
		float current_frame;
		float frame_anim_speed;
		int frame, end_frame1, flags1;
		// figure out where we are now
		if (G2_Get_Bone_Anim_Index(blist, index, current_time, &current_frame, &frame, &end_frame1, &flags1, &frame_anim_speed,
			numFrames))
		{
	...
	if (set_frame != -1)
	{
		blist[index].startTime = current_time - (set_frame - static_cast<float>(startFrame)) * 50.0 / anim_speed;
	}
	else
	{
...
// given a model, bone name, a bonelist, a start/end frame number, a anim speed and some anim flags, set up or modify an existing bone entry for a new set of anims
qboolean G2_Set_Bone_Anim(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* bone_name, const int startFrame,
	const int endFrame, const int flags, const float anim_speed, const int current_time,
...
qboolean G2_Get_Bone_Anim_Range_Index(const boneInfo_v& blist, const int bone_index, int* startFrame, int* endFrame)
{
	if (bone_index != -1)
	{
		...
		{
			*startFrame = blist[bone_index].startFrame;
			*endFrame = blist[bone_index].endFrame;
			return qtrue;
		}
	}
	return qfalse;
	...
	const int numFrames)
{
	...
	if (index >= 0 && !(index >= static_cast<int>(blist.size()) || blist[index].boneNumber == -1))
	{
		...
		if (blist[index].flags & (BONE_ANIM_OVERRIDE_LOOP | BONE_ANIM_OVERRIDE))
		{
			...
			if (retAnimSpeed)
			{
				*retAnimSpeed = blist[index].anim_speed;
			}
			return qtrue;
		}
	}
...
qboolean G2_Pause_Bone_Anim_Index(boneInfo_v& blist, const int bone_index, const int current_time, const int numFrames)
{
	if (bone_index >= 0 && bone_index < static_cast<int>(blist.size()))
	{
		...
		if (blist[bone_index].pauseTime)
		{
			...
			if (G2_Get_Bone_Anim_Index(blist, bone_index, blist[bone_index].pauseTime, &current_frame, &startFrame,
				&endFrame, &flags, &anim_speed, numFrames))
			{
				// reset start time so we are actually on this frame right now
				G2_Set_Bone_Anim_Index(blist, bone_index, startFrame, endFrame, flags, anim_speed, current_time,
					current_frame, 0, numFrames);
				// no pausing anymore
...
qboolean G2_Pause_Bone_Anim(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* bone_name, const int current_time)
{
	...
	return G2_Pause_Bone_Anim_Index(blist, index, current_time, ghlInfo->aHeader->numFrames);
}
...
#if _DEBUG
	//	G2_Bone_Not_Found(bone_name,ghlInfo->mFileName);
#endif
	// didn't find it
	return -1;
}
	...
	const int arg_end_frame, const int flags, const float anim_speed)
{
	...
	if (index != -1)
	{
		...
		//		blist[index].lastTime = blist[index].startTime = (current_time - (((setFrame - (float)startFrame) * 50.0)/ anim_speed));
		blist[index].flags &= ~(BONE_ANIM_TOTAL);
		blist[index].flags |= mod_flags;
	...
	if (index != -1)
	{
		...
		blist[index].pauseTime = 0;
		//		blist[index].boneMap = NULL;
		//		blist[index].lastTime = blist[index].startTime = (current_time - (((setFrame - (float)startFrame) * 50.0f)/ anim_speed));
		blist[index].flags &= ~(BONE_ANIM_TOTAL);
		blist[index].flags |= mod_flags;
	...
	sharedSetBoneIKStateParams_t* params)
{
	...
	*/
	...
	G2_Set_Bone_Anim_No_BS(g2, rmod_a, blist, bone_name, startFrame, endFrame - 1,
		BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
		1.0f);
